...
class BaselineDecisionTreeClassifier:
    ...
    def _cond_entropy(self, idx, feature_id):
        ...
        feature_vals, val_counts = np.unique(self.X[idx, feature_id], return_counts=True)
        ...
        return feature_vals[best_id], H_cond[best_id], equal_idx[best_id], n_equal_idx[best_id]

    def _info_gain(self, idx, feature_id):
        # claculate information gain if split target feature
        H = self._entropy(idx)

        best_split_val, best_H_cond, best_equal_idx, best_n_equal_idx = self._cond_entropy(idx, feature_id)
    ...
    def _get_best_feature(self, idx, feature_ids):
        ...
        best_split_vals, best_IGs, best_equal_idx, best_n_equal_idx = map(
            list, zip(*[self._info_gain(idx, feature_id) for feature_id in feature_ids])
        )
    ...
    def _fit_rec(self, idx, feature_ids, node):
        ...
        if np.unique(self.y[idx]).size == 1:
            ...
            desc = "N: {}\nclass: {}".format(node.n_samples, node.label)
            node.desc = desc
            return node
        ...
        best_feature_id, best_split_value, best_IG, best_equal_idx, best_n_equal_idx = self._get_best_feature(
            idx, feature_ids
        )
        ...
        desc = "{} == {}\nentropy: {:.2f}\nN: {}".format(
            node.split_feature, node.split_value, node.entropy, node.n_samples
        )
        node.desc = desc
        ...
        if len(np.unique(self.X[best_equal_idx, best_feature_id])) == 1:
            feature_ids_left = np.delete(feature_ids, np.where(feature_ids == best_feature_id)[0])
        ...
        if len(np.unique(self.X[best_n_equal_idx, best_feature_id])) == 1:
            feature_ids_right = np.delete(feature_ids, np.where(feature_ids == best_feature_id)[0])
    ...
    def plot_tree(self):
        ...
        nx.draw_networkx(self.G, pos=pos, labels=labels, arrows=True, bbox=dict(facecolor="white"))
    ...
    def _hierarchy_pos(self, G, root, levels=None, width=1.0, height=1.0):
        ...
        if levels is None:
            ...
            levels = {cur_l: {TOTAL: levels[cur_l], CURRENT: 0} for cur_l in levels}
        vert_gap = height / (max([cur_l for cur_l in levels]) + 1)
        return make_pos({})

    def _traverse(self, node, node_num):

        if node.type == "terminal":
            self.G.add_node(node_num, type=node.type, n_samples=node.n_samples, label=node.label, desc=node.desc)
            return node_num
        else:
            self.G.add_node(
                node_num,
