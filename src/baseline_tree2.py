...
class DecisionTreeClassifier:
    ...
    def _info_gain(self, idx, feature_id):
        ...
        best_H_cond, \
        best_equal_idx, \
        best_n_equal_idx = self._cond_entropy(idx, feature_id)
    ...
    def _get_best_feature(self, idx, feature_ids):
        ...
        best_IGs, \
        best_equal_idx, \
        best_n_equal_idx= map(list, zip(*[self._info_gain(idx, feature_id)
                                          for feature_id in feature_ids]))

        best_feature_idx = np.argmax(best_IGs)
        
        return feature_ids[best_feature_idx], \
    ...
    def _fit_rec(self, idx, feature_ids, node):
        ...
        best_IG, \
        best_equal_idx, \
        best_n_equal_idx = self._get_best_feature(idx, feature_ids)
        ...
        if len(np.unique(self.X[best_equal_idx, best_feature_id])) == 1:
            feature_ids_left = np.delete(feature_ids,
        ...
        if len(np.unique(self.X[best_n_equal_idx, best_feature_id])) == 1:
            feature_ids_right = np.delete(feature_ids,
                                          np.where(feature_ids==best_feature_id)[0])
    ...
    def _hierarchy_pos(self, G, root, levels=None, width=1., height=1.):
        TOTAL = "total"
        CURRENT = "current"
